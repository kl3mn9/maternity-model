...
freqZ.long <- gather(freqZ[178:270,], Facility, NumBirths, count1:count35)
freqZ_comp.long <- gather(freqZ_comp[178:270,], Facility, CompBirths, count1:count7)
...
calcFacStats <- function(params, FT, breaks)
{
	...
	freqLabourZHour <- FT$getOccupancyByCentreSnapshot2(labour_start, labour_end, breaks, facility, facility.numbers)
...
df.overSBA.long$Type <- rep("BEmOC", nrow(df.overSBA.long))
...
df.overLW.long$Type <- factor(df.overLW.long$Type, levels=c("CEmOC", "BEmOC"))
...
p7sum <- ggplot(df.overLW.long, aes(x=Type, y=PctTimeOverLWCapacity, fill=Type)) + theme_classic() + geom_boxplot(colour="black") + 
	...
	theme(axis.title.x=element_blank(), axis.title.y=element_text(size=16), axis.text=element_text(size=16), legend.position="none", plot.margin=margin(0,0,0,1, "cm"))
print(p7sum)


df.emptyLW.long$Type <- factor(df.emptyLW.long$Type, levels=c("CEmOC", "BEmOC"))
...
df.overSBA.long$Type <- factor(df.overSBA.long$Type, levels=c("CEmOC", "BEmOC"))
...
df.overSBA4.long$Type <- factor(df.overSBA4.long$Type, levels=c("CEmOC", "BEmOC"))
